...
mainRepoName="cdss-app-aug3-fortran"
...
#mainRepoFolder="$HOME/cdss-dev/AUG3/git-repos/${mainRepoName}"
...
# Get the software version from the aug4_db.for file code:
# data ver        /' 0.81'/
programVersionFile="${mainRepoFolder}/src/fortran/aug4_db.for"
if [ ! -f "${programVersionFile}" ]; then
	echo ""
	echo "AUG3 version file does not exist:"
...
programVersion=`cat ${programVersionFile} | grep 'data ver' | head -1 | cut -d '/' -f 2 | tr -d ' ' | tr -d "\'"`
echo ""
echo "AUG3 version detected to be ${programVersion}"
...
# If it does not exist, create an output folder for the AUG3 version
doxygenWorkFolder="${doxygenProjectFolder}/doxygen-aug3-${programVersion}"
doxygenWorkFolderRelative="doxygen-aug3-${programVersion}"
if [ ! -d "${doxygenWorkFolder}" ]; then
	echo ""
	echo "Creating Doxygen working folder for AUG3 version:"
